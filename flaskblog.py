...
class User(db.Model):
    ...
    image_file=db.Column(db.String(20),nullable=False,default='default.jpg')#default-default dp
    password=db.Column(db.String(60),nullable=False)
    #posts has a one to many relationship with the Post model
    #backref-author attribute can be used to get the user who created the post
    #lazy- relationship that defines when SQL alchemy loads data from the database. If true sql alchemy will load data in one go
    posts=db.relationship('Post',backref='author',lazy=True)
   

    def __repr__(self):
        return f"User('{self.username}','{self.email}','{self.image_file}')"   #this is how the object is printed
 
class Post(db.Model):
    id=db.Column(db.Integer,primary_key=True)
    title=db.Column(db.String(100),nullable=False)
    date_posted=db.Column(db.DateTime,nullable=False,default=datetime.utcnow) #default datetime is current one
